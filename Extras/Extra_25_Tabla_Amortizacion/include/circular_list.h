...
template<typename T>
void CircularList<T>::push_at(const T& value, int index) {
...
template<typename T>
Node<T> *CircularList<T>::at(int index) {
    if (index < 0 || index >= _size) {
        throw std::invalid_argument("index out of bounds");
    }
...
template <typename T>
void CircularList<T>::remove_at(int index) {
    if (index < 0 || index >= _size) {
        throw std::invalid_argument("index out of bounds");
    }
...
template<typename T>
void CircularList<T>::for_each(std::function<void(Node<T>*, int)> callback) {
    Node<T> *current = head;
    int index = 0;
...
template<typename T>
void CircularList<T>::for_each(std::function<void(Node<T>*)> callback) {
...
template<typename T>
void CircularList<T>::for_each(std::function<void(T)> callback) {
    for_each([&](Node<T> *node, int index) {
        callback(node->get_data());
    });
}
...
template<typename T>
void CircularList<T>::for_each(std::function<void(T, int)> callback) {
    for_each([&](Node<T> *node, int index) {
...
template<typename T>
void CircularList<T>::until(std::function<bool(Node<T>*)> callback) {
    Node<T> *current = head;
...
template<typename T>
void CircularList<T>::until(std::function<bool(T)> callback) {
    until([&](Node<T> *node) {
...
template<typename T>
Node<T> *CircularList<T>::find(std::function<bool(Node<T> *, int)> callback) {
    Node<T> *current = head;
    int index = 0;
...
template<typename T>
Node<T> *CircularList<T>::find(std::function<bool(Node<T> *)> callback) {
...
template<typename T>
Node<T> *CircularList<T>::find(std::function<bool(T)> callback) {
    return find([&](Node<T> *node, int index) -> bool {
        return callback(node->get_data());
        });
}
...
template<typename T>
Node<T> *CircularList<T>::find(std::function<bool(T, int)> callback) {
    return find([&](Node<T> *node, int index) -> bool {
